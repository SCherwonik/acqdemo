...
| ~~`acqdemo:log`~~ | Dropped before release: `acqdemo:extract` covers gathering, and a single win can be added to the ledger directly | | | |
...
- Monthly 5-minute ramble, added straight to the ledger, plus after any briefing, release or praise email.
...
| Monthly | 5-minute capture | add a ledger entry |
...
2. **Wave 2:** `acqdemo:midpoint` and `acqdemo:plan`, each with its own tests. A capture skill was built and then dropped; see Section 3.2.
