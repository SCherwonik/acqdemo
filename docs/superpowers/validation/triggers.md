...
| 4 | Pull what I did this year from git and my calendar | acqdemo:harvest | PASS |
...
Where a prompt competes with other installed skills, saying "AcqDemo" or invoking the skill directly (`/acqdemo:harvest`) always works.

## Rechecked after the skill rename (0.3.0)
Skills are now `acqdemo:start`, `acqdemo:annual`, `acqdemo:midpoint`, `acqdemo:plan`, `acqdemo:log`, `acqdemo:harvest`, and `acqdemo:review`. All eleven prompts were rerun against the renamed skills.
...
- Harness: an early version waited for each session to finish and timed out on prompt 4 even though `acqdemo:harvest` had loaded in 7 seconds; the harness now streams events and stops at the first `Skill` call.
