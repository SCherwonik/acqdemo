...
1. Position duties: read them with `../../plan/scripts/prd_text.py --file "<profile prd_path>"` and ask about the two or three duties that carry the most weight. "Your PRD duty 3 is <duty>. What did you do against that this cycle?"
...
Assume every answer arrives as dictation. Speech-to-text garbles domain terms, acronyms, program names, tool names, and people's names, and it does not garble the same word the same way twice.

### Resolve before asking
Before asking the user to repeat or spell a garbled word, check it against material that already exists:
- The workspace documents: prior assessments, the midpoint, the contribution plan, the PRD.
- Harvest snapshots in `FY<yy>/harvest/`.
- Repository names, file names, and folder names.
...
Apply the match silently in the ledger and in notes; do not make the user watch the correction happen. Say the corrected term back once, in the next message, so the user can object: "I have this as CAS2Net, the system named in your workspace." Move on unless the user corrects it.

### Never guess a name
A person's name never gets the silent-correction treatment. Confirm it against `roster.md`, or ask the user to spell it. Names go into private notes and the ledger only, and a wrong name there is worse than one left unresolved.

### Never drop a garbled term
If a garbled word cannot be matched against any source, do not drop it and move on silently. Quote it back exactly as heard and ask what it was: "I heard '<word>' and could not match it to anything in your workspace. What was that?"

### Numbers by ear
Numbers spoken in words ("a hundred and twenty", "fifteen hundred") are confirmed as digits before they enter the ledger: "I have that as 120, correct?"

Every saved ramble (`FY<yy>/rambles/<date>-<topic>.md`) ends with its own `## Transcription glossary` section, heard -> meant, one line per correction:
```
## Transcription glossary
- heard: "re gres sion tool" -> meant: Regression Tool
- heard: "cas net" -> meant: CAS2Net
```
Later sessions and other skills read this glossary first, so the same word is never re-resolved from scratch.
