...
Read `../start/references/ledger-format.md` for the entry format. Draft the entry from the ramble, then ask at most six targeted questions, only for fields still missing, in this order:
...
## Rules
- Treat every ramble as dictation: resolve garbled domain terms and program names against the user's own material before asking, and never invent or drop an unmatched one (question bank, Voice and transcription).
