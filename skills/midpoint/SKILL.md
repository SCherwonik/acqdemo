...
Reuses `../annual/SKILL.md` end to end for the mechanics: Load, Gate,
...
## References
- Annual engine (step names below refer to this): `../annual/SKILL.md`
- Checker: `../review/scripts/check.py`
- Pay pool overlay fields: `../start/templates/paypool.md`
- Program rules (Midpoint, Closeout sections): `../start/references/rules/ccas-core.md`
...
The annual's operating rules apply unchanged, including **Save before you ask** (write `session-state.md` before any message that waits on the user) and voice transcription handling (question bank, Voice and transcription).

## Differences from the annual (`../annual/SKILL.md`)
...
7. **Review command.** In Step 7, call the checker directly with midpoint flags
   ...
   checks (`../review/SKILL.md` Step 2) as the annual does:
   ```
   python ../review/scripts/check.py --dir <final folder> \
