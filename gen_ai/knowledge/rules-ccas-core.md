...
Tags: **[common]** applies across AcqDemo per the program's Federal Register notice and Operating Guide as reflected in pay pool guidance. **[pay pool]** comes from one pay pool's business rules or guidance (the examples in this pack) and may differ in yours. The user's `paypool.md` overrides anything tagged [pay pool]. When a rule matters to a decision, tell the user to confirm it against their pay pool's current business rules.
...
- [common] The employee writes a midpoint self-assessment in C-R-I format. The business rules set no per-factor minimum for midpoints; this toolkit checks for at least one per factor unless `paypool.md` `min_entries_midpoint` says otherwise.
...
- [common] C-R-I format (Contribution, Result, Impact) for every entry. Some pay pool guidance and business rules (including the example 2026 guidance) still call the same structure W-R-I (What, Result, Impact); the labels differ, not the content. Read the preferred label from `paypool.md` `what_label`.
...
- [pay pool] OCS of 94 and above is generally attainable for technical advisors, branch chiefs, and division chiefs; NH-IV employees consistently providing strategic contributions are also eligible. Calibration checks that categorical scores match the descriptors (the example rules: "4M should show strategic contributions").
